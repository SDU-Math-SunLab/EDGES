function [W1, W2, H1, H2, p1, p2] = EDGES(X,Y,Z,L1,lambda1,lambda2,seita1,seita2,tol,d,iterMax)
% <INPUT>
%        X: ST data
...
%        seita1:decomposition hyper-parameter balancing reconstruction error and constraints for the ST data
%        seita2:decomposition hyper-parameter for the unique scRNA-seq data
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    [delta_init1,~,~,~] = original_loss(W1,W2,H1,H2,X,Y,Z,L1,lambda1,lambda2,seita1,seita2,d);
    delta2 = delta_init1;
    ...
    total_lossall = [];   
       ...
       %% multiplicative update algorithm
        for iter = 1:iterMax
            ...
            w1 = W1.*((seita1*X1H1t + X2H2t)./(seita1*W1H1H1t + W1H2H2t));
            W1 = w1;
                                                         
            [W1tX1,W1tW1H1,eddH1,H1L1] = H1_update(W1,H1,X,L1,d);
            h1 = H1.*((seita1*W1tX1 - lambda1*H1L1)./(seita1*W1tW1H1 + lambda2*eddH1));
            H1 = h1;
            ...
            h2 = H2.*((W1tX2 + seita2*W2tX3)./(W1tW1H2 + seita2*W2tW2H2 + lambda2*eddH2));            
            H2 = h2;  
          ...
          %% convergence monitoring
            ...
            % loss calculation
            total_loss = seita1*X1_loss + X2_loss + seita2*X3_loss+ lambda1*H1_loss + lambda2*H_total_loss; 
